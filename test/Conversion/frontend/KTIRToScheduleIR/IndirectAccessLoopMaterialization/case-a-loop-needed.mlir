...
// Input is the output of IndirectAddrBufLegalization (Phase 2): a 1-D IAB
// (capacity already satisfied, W = 0) with a single entry loop already
// materialized. Two remaining direct-subscript intermediate variables index
// %desc_1 directly: %arg7 (dim 0, stride 4096) and %arg8 (dim 1, stride 1,
// full 64-element coverage).
//
// %arg7 fails dense-packing: size[1]*stride[1] = 64*1 = 64 != 4096, so it is
// not the innermost dimension and its neighbor isn't contiguous with it.
// Phase 3 must materialize a loop for it, leaving %arg8 retained (it passes
// both dense-packing, being innermost with stride 1, and full coverage,
// since its trip count equals %desc_1's dim-1 size of 64).
//
// The output descriptor access tile (%desc_2_at) already has its leading
// dimension pinned by Phase 2 (to %i2, the entry loop IV) — Phase 3 must
// continue pinning at the next dimension, not restart from 0.

// CHECK-LABEL: func.func @case_a
...
// CHECK:           tensor.expand_shape {{.*}} tensor<64xf16> into tensor<1x1x64xf16>
...
#set_base = affine_set<(d0, d1) : (d0 >= 0, -d0 + 63 >= 0, d1 >= 0, -d1 + 63 >= 0)>
...
module @local_schedule_1 {
  ...
  func.func @case_a() attributes {grid = [1 : index]} {
    ...
    %iab_mv_final = scf.for %i2 = %c0_i2 to %c32 step %c1_i2
      ...
      %tmp1 = ktdp_lowering.construct_indirect_access_tile
          ...
          : memref<64x64xf16>, memref<32xindex, "IAB"> -> !ktdp.access_tile<2x64xindex>
      ...
      // Phase 2 already pinned this access tile's leading dim to %i2 (rather
      // than dropping it), so a tensor.expand_shape bridges the compute
      // chain's own (drop-narrowed) rank-2 result back to the tile's
      // pinned rank-3 shape before the store.
